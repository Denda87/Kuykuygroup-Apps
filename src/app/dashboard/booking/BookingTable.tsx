...
export default function BookingTable({ bookings }: { bookings: Booking[] }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #D4AF3730" }}>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-gray-600"><p className="text-sm">Belum ada reservasi</p></div>
      ) : (
          ...
          <tbody>
            {filtered.map((b, i) => (
              <tr key={b.id} style={{ borderBottom: "1px solid #D4AF3710", background: i % 2 === 0 ? "#141200" : "#111000" }} className="hover:bg-[#D4AF3710]">
                ...
                <td className="px-4 py-4"><p className="text-gray-200 font-medium">{b.customer_name}</p><p className="text-gray-600">{b.phone}</p></td>
                <td className="px-4 py-4"><p className="text-gray-300">{b.services?.name || "—"}</p>{b.services?.price_idr && <p className="text-[#D4AF37]" style={{ fontSize: 10 }}>Rp {b.services.price_idr.toLocaleString("id-ID")}</p>}</td>
                <td className="px-4 py-4 text-gray-400">{b.branches?.name || "—"}</td>
                <td className="px-4 py-4"><span className="px-2 py-0.5 rounded-full text-white font-semibold" style={{ fontSize: 9, background: STATUS_COLORS[b.status] || "#666" }}>{STATUS_LABELS[b.status]}</span></td>
                <td className="px-4 py-4">
                  <select disabled={updating === b.id} value={b.status} onChange={e => updateStatus(b.id, e.target.value as Booking["status"])} className="text-[10px] rounded-lg px-2 py-1 focus:outline-none disabled:opacity-50" style={{ background: "#1a1600", color: "#D4AF37", border: "1px solid #D4AF3740" }}>
