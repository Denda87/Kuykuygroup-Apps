      ...
      <div className="px-4 py-4 flex flex-col gap-3">
        ...
        <div className="rounded-2xl p-6 text-center relative" style={{ background: "linear-gradient(135deg,#1e1800,#141000,#1a1400)", border: "1px solid #D4AF3745" }}>
          ...
          <div className="relative inline-block mb-4">
            <div className="w-24 h-24 rounded-full flex items-center justify-center text-black font-bold text-3xl font-serif" style={{ background: "linear-gradient(135deg,#C9A84C,#f5e070,#D4AF37,#B8960C)", boxShadow: "0 0 28px #D4AF3760" }}>
              {name.charAt(0)}
            </div>
            <button className="absolute -bottom-1 -right-1 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "#D4AF37" }}>
              <Camera size={14} color="#000" />
            </button>
          </div>

          {editMode ? (
            <input value={name} onChange={e => setName(e.target.value)} className="bg-transparent text-center text-xl font-bold font-serif text-white border-b focus:outline-none w-full mb-2" style={{ borderColor: "#D4AF37" }} />
          ) : (
          ...
          <div className="flex gap-2 justify-center flex-wrap mt-2">
            {roles.map(role => (
              <button key={role} onClick={() => setSelectedRole(role)} className="px-3 py-1.5 rounded-full text-xs font-semibold"
                style={selectedRole === role
                  ? { background: "linear-gradient(135deg,#C9A84C,#D4AF37,#B8960C)", color: "#000", boxShadow: "0 2px 10px #D4AF3750" }
          ...
          {editMode && (
            <button onClick={() => setEditMode(false)} className="mt-4 px-6 py-2 rounded-xl text-xs font-bold text-black" style={{ background: "linear-gradient(135deg,#C9A84C,#D4AF37)" }}>
              SIMPAN PERUBAHAN
            </button>
          )}
        </div>
