...
export default function BookingForm({ services, branches }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (err) { setError("Terjadi kesalahan. Silakan coba lagi."); } else { setSuccess(true); }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <button type="submit" disabled={loading} className="w-full py-4 font-bold text-black rounded-2xl text-sm uppercase tracking-[0.2em] disabled:opacity-60 mt-2" style={{ background: "linear-gradient(135deg,#C9A84C,#f5e070,#D4AF37,#B8960C)", boxShadow: "0 6px 28px #D4AF3750" }}>
